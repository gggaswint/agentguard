...
def _require_mcp_sdk() -> bool:
    ...
    print(
        ...
        'on Python 3.10+: pip install "aegize[mcp]"',
        file=sys.stderr,
    )
    return False
...
async def _run_inspect(upstream_argv: list[str], startup_timeout: float) -> None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "inspect":
        ...
        try:
            ...
            return 2
        try:
            asyncio.run(_run_inspect(upstream_argv, args.startup_timeout))
        except GatewayError as exc:
            print(f"aegize-mcp error: {exc}", file=sys.stderr)
            return 1
        return 0
